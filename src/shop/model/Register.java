...
public class Register {
	...
	/* 생성자의 이름은 같고, 매개변수의 개수를 다르게 해서 표현하는 것을 생성자 오버로딩 */
	...
	public void setUserid(String uid) {
		...
		} else { // 객체명.length(); 배열 길이 값을 구할 때
			if (uid.length() < 8) {
				System.out.println("아이디 길이가 너무 짧습니다.\n아이디는 8자리 이상 12 이하로 입력해 주세요");
	...
	public void setPasswd(String pwd) {
		if (!pwd.equals("") && pwd.length() >= 8) { // 빈값x : 입력 -> 8 이상 : 정상
			System.out.println("사용하셔도 되는 비밀번호 입니다.\n로그인 페이지로 이동합니다.");
		} else {
	...
	// method : 공통작업 또는 관리를 쉽게 하기 위해서(동작) - 유지/보수를 편하게 하기 위해서	
	public String disp() {
